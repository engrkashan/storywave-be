 ...
 */
export function planAtomicBeats(rawBeats = []) {
  logger.info(`🔬 [Atomic Beat Planner] Processing ${rawBeats.length} raw beats...`);
  ...
  for (let i = 0; i < rawBeats.length; i++) {
    ...
    // Check if the beat contains multiple compound action verbs separated by conjunctions or commas
    // e.g. "jumped over the wall, looked behind him and ran" -> split into 3 beats
    const compoundParts = splitCompoundAction(actionText);
    ...
    if (compoundParts.length > 1) {
      ...
      compoundParts.forEach((part, subIdx) => {
        atomicBeats.push({
          ...
          narrative: part,
          action: part,
          // Only attach spoken text to the first sub-beat to avoid duplicate speech
